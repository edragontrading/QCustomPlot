...
void QCustomPlot::setOpenGl(bool enabled, int multisampling) {
    ...
    if (mOpenGl) {
        ...
        if (mAntialiasedElements == QCP::aeAll) setAntialiasedElements(mOpenGlAntialiasedElementsBackup);
        if (!mPlottingHints.testFlag(QCP::phCacheLabels)) setPlottingHint(QCP::phCacheLabels, mOpenGlCacheLabelsBackup);
        freeOpenGl();
    }
...
bool QCustomPlot::removeLayer(QCPLayer *layer) {
    ...
    if (QSharedPointer<QCPAbstractPaintBuffer> pb = layer->mPaintBuffer.toStrongRef()) pb->setInvalidated();
...
bool QCustomPlot::moveLayer(QCPLayer *layer, QCPLayer *otherLayer, QCustomPlot::LayerInsertMode insertMode) {
    ...
    if (QSharedPointer<QCPAbstractPaintBuffer> pb = layer->mPaintBuffer.toStrongRef()) pb->setInvalidated();
    if (QSharedPointer<QCPAbstractPaintBuffer> pb = otherLayer->mPaintBuffer.toStrongRef()) pb->setInvalidated();
    ...
    while (searchSubElements && currentElement) {
        ...
        foreach (QCPLayoutElement *subElement, currentElement->elements(false)) {
            if (subElement && subElement->realVisibility() && subElement->selectTest(pos, false) >= 0) {
                currentElement = subElement;
                searchSubElements = true;
                if (QCPAxisRect *ar = qobject_cast<QCPAxisRect *>(currentElement)) result = ar;
                break;
            }
        }
    }
...
void QCustomPlot::replot(QCustomPlot::RefreshPriority refreshPriority) {
    ...
    if (!qFuzzyIsNull(mReplotTimeAverage))
        mReplotTimeAverage = mReplotTimeAverage * 0.9 + mReplotTime * 0.1;  // exponential moving average with a
                                                                            // time constant of 10 last replots
    else
        mReplotTimeAverage = mReplotTime;  // no previous replots to average with, so
                                           // initialize with replot time
...
void QCustomPlot::resizeEvent(QResizeEvent *event) {
    ...
    setViewport(rect());
    replot(rpQueuedRefresh);  // queued refresh is important here, to prevent painting
                              // issues in some contexts (e.g. MDI subwindow)
}
...
void QCustomPlot::mouseDoubleClickEvent(QMouseEvent *event) {
    ...
    event->accept();  // in case QCPLayerable reimplementation manipulates event
                      // accepted state. In QWidget event system, QCustomPlot wants
                      // to accept the event.
}
...
void QCustomPlot::mousePressEvent(QMouseEvent *event) {
    ...
    if (mSelectionRect && mSelectionRectMode != QCP::srmNone) {
        if (mSelectionRectMode != QCP::srmZoom ||
            qobject_cast<QCPAxisRect *>(axisRectAt(mMousePressPos)))  // in zoom mode only activate selection
                                                                      // rect if on an axis rect
        ...
        QList<QCPLayerable *> candidates = layerableListAt(mMousePressPos, false, &details);
        if (!candidates.isEmpty()) {
            mMouseSignalLayerable = candidates.first();  // candidate for signal emission is always topmost
...
void QCustomPlot::mouseReleaseEvent(QMouseEvent *event) {
    ...
    if (!mMouseHasMoved)  // mouse hasn't moved (much) between press and release,
                          // so handle as click
    {
        if (mSelectionRect && mSelectionRect->isActive())  // a simple click shouldn't successfully finish a
...
void QCustomPlot::setupPaintBuffers() {
    int bufferIndex = 0;
    if (mPaintBuffers.isEmpty()) mPaintBuffers.append(QSharedPointer<QCPAbstractPaintBuffer>(createPaintBuffer()));
    ...
    for (int layerIndex = 0; layerIndex < mLayers.size(); ++layerIndex) {
        ...
        if (layer->mode() == QCPLayer::lmLogical) {
            ...
            if (layerIndex < mLayers.size() - 1 &&
                mLayers.at(layerIndex + 1)->mode() == QCPLayer::lmLogical)  // not last layer, and next one is logical,
...
void QCustomPlot::processRectSelection(QRect rect, QMouseEvent *event) {
    ...
    typedef QMultiMap<int, SelectionCandidate> SelectionCandidates;  // map key is number of selected data points, so we
...
bool QCustomPlot::registerPlottable(QCPAbstractPlottable *plottable) {
    ...
    if (!plottable->layer())  // usually the layer is already set in the constructor of
...
QCPLayerable *QCustomPlot::layerableAt(const QPointF &pos, bool onlySelectable, QVariant *selectionDetails) const {
    ...
    if (selectionDetails && !details.isEmpty()) *selectionDetails = details.first();
    if (!candidates.isEmpty())
...
QPixmap QCustomPlot::toPixmap(int width, int height, double scale) {
    ...
    if (painter.isActive()) {
        ...
        if (!qFuzzyCompare(scale, 1.0)) {
            if (scale > 1.0)  // for scale < 1 we always want cosmetic pens where possible,
                              // because else lines might disappear for very small scales
                painter.setMode(QCPPainter::pmNonCosmetic);
            painter.scale(scale, scale);
        }
        if (mBackgroundBrush.style() != Qt::SolidPattern &&
            mBackgroundBrush.style() != Qt::NoBrush)  // solid fills were done a few lines above with
                                                      // QPixmap::fill
