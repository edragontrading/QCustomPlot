...
void QCPPolarLegendItem::draw(QCPPainter *painter) {
    ...
    QRectF textRect =
        painter->fontMetrics().boundingRect(0, 0, 0, iconSize.height(), Qt::TextDontClip, mPolarGraph->name());
...
void QCPPolarGraph::addData(const QVector<double> &keys, const QVector<double> &values, bool alreadySorted) {
    ...
    mDataContainer->add(tempData, alreadySorted);  // don't modify tempData beyond
                                                   // this to prevent copy on write
}
...
void QCPPolarGraph::rescaleKeyAxis(bool onlyEnlarge) const {
    ...
    if (foundRange) {
        ...
        {
            double center =
                (newRange.lower + newRange.upper) * 0.5;  // upper and lower should be equal anyway, but just to make
...
void QCPPolarGraph::draw(QCPPainter *painter) {
    ...
    for (int i = 0; i < allSegments.size(); ++i) {
        ...
        QCPDataRange lineDataRange =
            isSelectedSegment
                ? allSegments.at(i)
                : allSegments.at(i).adjusted(-1, 1);  // unselected segments extend lines to bordering
...
void QCPPolarGraph::selectEvent(QMouseEvent *event, bool additive, const QVariant &details,
    ...
    if (mSelectable != QCP::stNone) {
        ...
        if (additive) {
            ...
            {
                if (mSelection.contains(newSelection))  // if entire newSelection is already selected,
                                                        // toggle selection
            ...
            setSelection(newSelection);
        if (selectionStateChanged) *selectionStateChanged = mSelection != selectionBefore;
    }
}
...
void QCPPolarGraph::drawScatterPlot(QCPPainter *painter, const QVector<QPointF> &scatters,
    ...
    for (int i = 0; i < scatters.size(); ++i) style.drawShape(painter, scatters.at(i).x(), scatters.at(i).y());
}
...
void QCPPolarGraph::getScatters(QVector<QPointF> *scatters, const QCPDataRange &dataRange) const {
    ...
    for (int i = 0; i < data.size(); ++i) {
        if (!qIsNaN(data.at(i).value)) (*scatters)[i] = valueAxis->coordToPixel(data.at(i).key, data.at(i).value);
    }
}
...
void QCPPolarGraph::getOptimizedLineData(QVector<QCPGraphData> *lineData,
    ...
    const double clipMargin = range.size() * 0.05;  // extra distance from visible circle, so optimized
                                                    // outside lines can cover more angle before having
                                                    // to place a dummy point to prevent tangents
    const double upperClipValue =
        range.upper +
        (reversed ? 0 : range.size() * 0.05 + clipMargin);  // clip slightly outside of actual range to avoid
    ...
    const double lowerClipValue = range.lower - (reversed ? range.size() * 0.05 + clipMargin
                                                          : 0);  // clip slightly outside of actual range to avoid
                                                                 // line thicknesses to peek into visible circle
    const double maxKeySkip = qAsin(qSqrt(clipMargin * (clipMargin + 2 * range.size())) / (range.size() + clipMargin)) /
    ...
    while (it != end) {
        if (it->value < lowerClipValue) {
            if (aboveRange)  // jumped directly from above to below visible range, draw
                             // previous point so entry angle is correct
            {
                aboveRange = false;
            ...
            if (it->key - skipBegin > maxKeySkip)  // add dummy point if we're exceeding the maximum
                                                   // skippable angle (to prevent unintentional intersections
                                                   // with visible circle)
            {
            ...
            if (belowRange)  // jumped directly from below to above visible range, draw
                             // previous point so entry angle is correct (if lower
        ...
        } else  // value within bounds where we don't optimize away points
        {
            if (aboveRange) {
                aboveRange = false;
                if (!reversed)
                    lineData->append(*(it - 1));  // just entered from above, draw previous point so
                                                  // entry angle is correct (if above means outer, so if
                                                  // not reversed axis)
            }
            if (belowRange) {
                belowRange = false;
                if (reversed)
                    lineData->append(*(it - 1));  // just entered from below, draw previous point so
                                                  // entry angle is correct (if below means outer, so if
                                                  // reversed axis)
            }
    ...
    if (aboveRange) {
        aboveRange = false;
        if (!reversed)
            lineData->append(*(it - 1));  // just entered from above, draw previous point so entry angle
                                          // is correct (if above means outer, so if not reversed axis)
    }
    if (belowRange) {
        belowRange = false;
        if (reversed)
            lineData->append(*(it - 1));  // just entered from below, draw previous point so entry angle
...
void QCPPolarGraph::getOptimizedScatterData(QVector<QCPGraphData> *scatterData,
    ...
    const double upperClipValue =
        range.upper + (reversed ? 0 : clipMargin);  // clip slightly outside of actual range to avoid
                                                    // scatter size to peek into visible circle
    const double lowerClipValue =
        range.lower - (reversed ? clipMargin : 0);  // clip slightly outside of actual range to avoid
                                                    // scatter size to peek into visible circle
    QCPGraphDataContainer::const_iterator it = begin;
    while (it != end) {
        if (it->value > lowerClipValue && it->value < upperClipValue) scatterData->append(*it);
        ++it;
    }
}
...
QVector<QPointF> QCPPolarGraph::dataToLines(const QVector<QCPGraphData> &data) const {
    ...
    for (int i = 0; i < data.size(); ++i) result[i] = mValueAxis->coordToPixel(data.at(i).key, data.at(i).value);
    return result;
}
