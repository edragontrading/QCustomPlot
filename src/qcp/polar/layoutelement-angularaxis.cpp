...
QCPPolarAxisAngular::QCPPolarAxisAngular(QCustomPlot *parentPlot)
    ...
    mLabelPainter.setCacheSize(24);  // so we can cache up to 15-degree intervals, polar angular axis uses
                                     // a bit larger cache than normal axes
...
QCPPolarAxisAngular::~QCPPolarAxisAngular() {
    ...
    for (int i = 0; i < radialAxesList.size(); ++i) removeRadialAxis(radialAxesList.at(i));
}
...
bool QCPPolarAxisAngular::removeRadialAxis(QCPPolarAxisRadial *radialAxis) {
    if (mRadialAxes.contains(radialAxis)) {
        ...
        qDebug() << Q_FUNC_INFO
                 << "Radial axis isn't associated with this angular axis:" << reinterpret_cast<quintptr>(radialAxis);
        return false;
    }
}
...
void QCPPolarAxisAngular::update(UpdatePhase phase) {
    ...
    switch (phase) {
        case upPreparation: {
            setupTickVectors();
            for (int i = 0; i < mRadialAxes.size(); ++i) mRadialAxes.at(i)->setupTickVectors();
            break;
        }
        case upLayout: {
            ...
            for (int i = 0; i < mRadialAxes.size(); ++i) mRadialAxes.at(i)->updateGeometry(mCenter, mRadius);
...
void QCPPolarAxisAngular::mousePressEvent(QMouseEvent *event, const QVariant &details) {
    ...
    if (event->buttons() & Qt::LeftButton) {
        ...
        if (mParentPlot->interactions().testFlag(QCP::iRangeDrag)) {
            ...
            for (int i = 0; i < mRadialAxes.size(); ++i) mDragRadialStart.append(mRadialAxes.at(i)->range());
        }
    }
}
...
bool QCPPolarAxisAngular::registerPolarGraph(QCPPolarGraph *graph) {
    ...
    if (!graph->layer())  // usually the layer is already set in the constructor of
