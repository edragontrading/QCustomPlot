...
bool QCPAxisRect::removeAxis(QCPAxis *axis) {
    ...
    while (it.hasNext()) {
        ...
        if (it.value().contains(axis)) {
            if (it.value().first() == axis && it.value().size() > 1)  // if removing first axis, transfer axis offset
                                                                      // to the new first axis (which at this point
                                                                      // is the second axis, if it exists)
                it.value()[1]->setOffset(axis->offset());
            mAxes[it.key()].removeOne(axis);
            if (qobject_cast<QCustomPlot *>(parentPlot()))  // make sure this isn't called from QObject dtor
...
QList<QCPGraph *> QCPAxisRect::graphs() const {
    ...
    foreach (QCPGraph *graph, mParentPlot->mGraphs) {
        if (graph->keyAxis()->axisRect() == this || graph->valueAxis()->axisRect() == this) result.append(graph);
    }
    return result;
}
...
void QCPAxisRect::updateAxesOffset(QCPAxis::AxisType type) {
    ...
    bool isFirstVisible = !axesList.first()->visible();  // if the first axis is visible, the second axis (which
...
void QCPAxisRect::layoutChanged() {
    if (mParentPlot && mParentPlot->axisRectCount() > 0 && mParentPlot->axisRect(0) == this) {
        if (axisCount(QCPAxis::atBottom) > 0 && !mParentPlot->xAxis) mParentPlot->xAxis = axis(QCPAxis::atBottom);
        if (axisCount(QCPAxis::atLeft) > 0 && !mParentPlot->yAxis) mParentPlot->yAxis = axis(QCPAxis::atLeft);
        if (axisCount(QCPAxis::atTop) > 0 && !mParentPlot->xAxis2) mParentPlot->xAxis2 = axis(QCPAxis::atTop);
        if (axisCount(QCPAxis::atRight) > 0 && !mParentPlot->yAxis2) mParentPlot->yAxis2 = axis(QCPAxis::atRight);
    }
}
    ...
    if (mParentPlot->interactions().testFlag(QCP::iRangeZoom)) {
        if (mRangeZoom != 0) {
            ...
            if (mRangeZoom.testFlag(Qt::Horizontal)) {
                ...
                foreach (QPointer<QCPAxis> axis, mRangeZoomHorzAxis) {
                    if (!axis.isNull()) axis->scaleRange(factor, axis->pixelToCoord(pos.x()));
                }
            }
            if (mRangeZoom.testFlag(Qt::Vertical)) {
                factor = qPow(mRangeZoomFactorVert, wheelSteps);
                foreach (QPointer<QCPAxis> axis, mRangeZoomVertAxis) {
                    if (!axis.isNull()) axis->scaleRange(factor, axis->pixelToCoord(pos.y()));
                }
            }
            mParentPlot->replot();
        }
