...
double QCPAxisTickerDateTime::getTickStep(const QCPRange &range) {
    double result = range.size() / double(mTickCount + 1e-10);  // mTickCount ticks on average, the small addition is to
                                                                // prevent jitter on exact integers

    mDateStrategy = dsNone;  // leaving it at dsNone means tick coordinates will
    ...
    {
        result =
            pickClosest(result, QVector<double>()
                                    << 1 << 2.5 << 5 << 10 << 15 << 30 << 60 << 2.5 * 60 << 5 * 60 << 10 * 60 << 15 * 60
                                    << 30 * 60 << 60 * 60  // second, minute, hour range
                                    ...
                                    << 86400 * 30.4375 * 12);  // day, week, month range (avg. days per month
                                                               // includes leap years)
...
int QCPAxisTickerDateTime::getSubTickCount(double tickStep) {
    ...
    switch (qRound(tickStep))  // hand chosen subticks for specific minute/hour/day/week/month
...
QVector<double> QCPAxisTickerDateTime::createTickVector(double tickStep, const QCPRange &range) {
    ...
    if (!result.isEmpty()) {
        if (mDateStrategy == dsUniformTimeInDay) {
            QDateTime uniformDateTime = keyToDateTime(mTickOrigin);  // the time of this datetime will be set
            ...
            QDateTime uniformDateTime = keyToDateTime(mTickOrigin);  // this day (in month) and time will be
                                                                     // set for all other ticks, if possible
            QDateTime tickDateTime;
            for (int i = 0; i < result.size(); ++i) {
                tickDateTime = keyToDateTime(result.at(i));
                tickDateTime.setTime(uniformDateTime.time());
                int thisUniformDay = uniformDateTime.date().day() <= tickDateTime.date().daysInMonth()
                                         ? uniformDateTime.date().day()
                                         : tickDateTime.date().daysInMonth();  // don't exceed month (e.g. try to set
                                                                               // day 31 in February)
                ...
                else if (thisUniformDay - tickDateTime.date().day() > 15)  // with leap years involved, date month may
                                                                           // jump backwards or forwards, and needs to
                                                                           // be corrected before setting day
                    tickDateTime = tickDateTime.addMonths(-1);
