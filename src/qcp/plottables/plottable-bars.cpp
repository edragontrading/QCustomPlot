...
double QCPBarsGroup::keyPixelOffset(const QCPBars *bars, double keyCoord) {
    ...
    if (index >= 0) {
        if (baseBars.size() % 2 == 1 && index == (baseBars.size() - 1) / 2)  // is center bar (int division on purpose)
        {
            ...
            for (int i = startIndex; i != index; i += dir)  // add widths and spacings of bars in between center and
                                                            // our bars
            {
...
QCPBars::~QCPBars() {
    ...
    if (mBarBelow || mBarAbove) connectBars(mBarBelow.data(),
...
QRectF QCPBars::getBarRect(double key, double value) const {
    ...
    if (qAbs(valuePixel - basePixel) <= qAbs(bottomOffset)) bottomOffset = valuePixel - basePixel;
    if (keyAxis->orientation() == Qt::Horizontal) {
...
double QCPBars::getStackedBaseValue(double key, bool positive) const {
    if (mBarBelow) {
        ...
        // one:
        double epsilon = qAbs(key) * (sizeof(key) == 4 ? 1e-6 : 1e-14);  // should be safe even when changed
        ...
        while (it != itEnd) {
            if (it->key > key - epsilon && it->key < key + epsilon) {
                if ((positive && it->value > max) || (!positive && it->value < max)) max = it->value;
            }
            ++it;
        }
