...
QCPGraph::QCPGraph(QCPAxis *keyAxis, QCPAxis *valueAxis)
    : QCPAbstractPlottable1D<QCPGraphData>(keyAxis, valueAxis), mLineStyle{}, mScatterSkip{}, mAdaptiveSampling{} {
...
void QCPGraph::draw(QCPPainter *painter) {
    ...
    // brushes:
    if (mSelectionDecorator) mSelectionDecorator->drawDecoration(painter, selection());
}
...
void QCPGraph::getLines(QVector<QPointF> *lines, const QCPDataRange &dataRange) const {
    ...
    if (mKeyAxis->rangeReversed() !=
        (mKeyAxis->orientation() == Qt::Vertical))  // make sure key pixels are sorted ascending in lineData
...
void QCPGraph::getScatters(QVector<QPointF> *scatters, const QCPDataRange &dataRange) const {
    ...
    if (mKeyAxis->rangeReversed() !=
        (mKeyAxis->orientation() == Qt::Vertical))  // make sure key pixels are sorted ascending in data
                                                    // (significantly simplifies following processing)
...
void QCPGraph::getOptimizedLineData(QVector<QCPGraphData> *lineData, const QCPGraphDataContainer::const_iterator &begin,
                                                     ...
                                                     // two points per pixel on average
    {
        ...
        int reversedRound = reversedFactor == -1 ? 1 : 0;  // is used to switch between floor (normal) and ceil
        ...
        bool keyEpsilonVariable =
            keyAxis->scaleType() == QCPAxis::stLogarithmic;  // indicates whether keyEpsilon needs to be
        ...
        while (it != end) {
            if (it->key < currentIntervalStartKey + keyEpsilon)  // data point is still within same pixel, so skip it
            ...
            } else  // new pixel interval started
            {
                                             ...
                                             // consolidate them to a cluster
                {
                    if (lastIntervalEndKey <
                        currentIntervalStartKey - keyEpsilon)  // last point is further away, so first point of
                    ...
                    if (it->key >
                        currentIntervalStartKey + keyEpsilon * 2)  // new pixel started further away from previous
        ...
        {
            if (lastIntervalEndKey <
                currentIntervalStartKey - keyEpsilon)  // last point wasn't a cluster, so first point of this
...
void QCPGraph::getOptimizedScatterData(QVector<QCPGraphData> *scatterData, QCPGraphDataContainer::const_iterator begin,
                                                     ...
                                                     // two points per pixel on average
    {
        ...
        while (it != end) {
            ...
            } else  // new pixel started
            {
                if (intervalDataCount >= 2)  // last pixel had multiple data points, consolidate them
                {
                    ...
                    int dataModulo = qMax(
                        1, qRound(intervalDataCount / (valuePixelSpan / 4.0)));  // approximately every 4 value pixels
        ...
        if (intervalDataCount >= 2)  // last pixel had multiple data points, consolidate them
        {
            ...
            int dataModulo =
                qMax(1,
                     qRound(intervalDataCount / (valuePixelSpan / 4.0)));  // approximately every 4 value pixels
                                                                           // one data point on average
            QCPGraphDataContainer::const_iterator intervalIt = currentIntervalStart;
            ...
            while (intervalIt != it) {
                ...
                if (!doScatterSkip)
                    ++intervalIt;
                else  // here we can't guarantee that adding scatterModulo doesn't exceed
                      // "it" (because "it" is equal to "end" here, and "end" isn't
...
QVector<QPair<QCPDataRange, QCPDataRange>> QCPGraph::getOverlappingSegments(QVector<QCPDataRange> thisSegments,
    ...
    if (thisData->isEmpty() || otherData->isEmpty() || thisSegments.isEmpty() || otherSegments.isEmpty()) return result;
    ...
    while (thisIndex < thisSegments.size() && otherIndex < otherSegments.size()) {
        ...
        if (bPrecedence <= 0)  // otherSegment doesn't reach as far as thisSegment, so continue with
            ...
            ++otherIndex;
        else  // otherSegment reaches further than thisSegment, so continue with next
              // thisSegment, keeping current otherSegment
            ++thisIndex;
    }
...
double QCPGraph::pointDistance(const QPointF &pixelPoint, QCPGraphDataContainer::const_iterator &closestData) const {
    ...
    if (mLineStyle != lsNone) {
        ...
        getLines(&lineData, QCPDataRange(0,
        ...
        const int step = mLineStyle == lsImpulse ? 2 : 1;  // impulse plot differs from other line styles in that the
