...
void MainWindow::setupSincScatterDemo(QCustomPlot *customPlot) {
    ...
    for (int i = 0; i < 250; ++i) {
        x0[i] = (i / 249.0 - 0.5) * 30 + 0.01;  // by adding a small offset we make sure not do divide by zero
                                                // in next code line
...
void MainWindow::setupScatterPixmapDemo(QCustomPlot *customPlot) {
    ...
    customPlot->plotLayout()->addElement(
        0, 0, new QCPTextElement(customPlot, "Regenerative Energies", QFont("sans", 12, QFont::Bold)));
    // axis configurations:
...
void MainWindow::setupMultiAxisDemo(QCustomPlot *customPlot) {
    ...
    customPlot->plotLayout()->insertRow(0);
    customPlot->plotLayout()->addElement(
        0, 0, new QCPTextElement(customPlot, "Way too many graphs in one plot", QFont("sans", 12, QFont::Bold)));
    // set labels:
...
void MainWindow::setupLogarithmicDemo(QCustomPlot *customPlot) {
    ...
    QVector<QCPGraphData> dataLinear(dataCount), dataMinusSinExp(dataCount), dataPlusSinExp(dataCount),
        dataFactorial(dataFactorialCount);
...
void MainWindow::setupItemDemo(QCustomPlot *customPlot) {
    ...
    phaseTracerArrow->startDir->setCoords(-40, 0);  // direction 30 pixels to the left of parent anchor
                                                    // (tracerArrow->start)
    ...
    phaseTracerArrow->setTail(QCPLineEnding(
        QCPLineEnding::esBar,
        (phaseTracerText->bottom->pixelPosition().y() - phaseTracerText->top->pixelPosition().y()) * 0.85));
    ...
    groupTracerArrow->startDir->setCoords(-40, 0);  // direction 30 pixels to the left of parent anchor
                                                    // (tracerArrow->start)
    ...
    groupTracerArrow->setTail(QCPLineEnding(
        QCPLineEnding::esBar,
        (groupTracerText->bottom->pixelPosition().y() - groupTracerText->top->pixelPosition().y()) * 0.85));
...
void MainWindow::setupAdvancedAxesDemo(QCustomPlot *customPlot) {
    ...
    customPlot->plotLayout()->addElement(1, 0, subLayout);     // sub layout in second row (grid layout will grow
                                                               // accordingly)
    ...
    QCPGraph *subGraphRandom =
        customPlot->addGraph(subRectLeft->axis(QCPAxis::atBottom), subRectLeft->axis(QCPAxis::atLeft));
...
void MainWindow::setupColorMapDemo(QCustomPlot *customPlot) {
    ...
    colorMap->data()->setRange(QCPRange(-4, 4), QCPRange(-4, 4));  // and span the coordinate range -4..4 in both key
                                                                   // (x) and value (y) dimensions
    ...
    for (int xIndex = 0; xIndex < nx; ++xIndex) {
        for (int yIndex = 0; yIndex < ny; ++yIndex) {
            ...
            z = 2 * x * (qCos(r + 2) / r - qSin(r + 2) / r);  // the B field strength of dipole radiation (modulo
                                                              // physical constants)
...
void MainWindow::setupFinancialDemo(QCustomPlot *customPlot) {
    ...
    ohlc->data()->set(QCPFinancial::timeSeriesToOhlc(time, value2, binSize / 3.0,
                                                     startTime));  // divide binSize by 3 just to make the ohlc bars a
                                                                   // bit denser
    ...
    customPlot->xAxis->setTickLabels(false);
    customPlot->xAxis->setTicks(false);  // only want vertical grid in main axis rect, so hide xAxis
...
void MainWindow::screenShot() {
    ...
    QPixmap pm =
        qApp->primaryScreen()->grabWindow(qApp->desktop()->winId(), this->x() + 2, this->y() + 2,
                                          this->frameGeometry().width() - 4, this->frameGeometry().height() - 4);
#elif QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
    QPixmap pm =
        qApp->primaryScreen()->grabWindow(qApp->desktop()->winId(), this->x() - 7, this->y() - 7,
